...
#define SET_BIT(array, x, y, width)   (array[((y) * (width) + (x)) >> 3] |= (MASK >> (((y) * (width) + (x)) % 8)))
// Turn off the bit (it establishes the state of the cell as DEAD = 0)
#define CLEAR_BIT(array, x, y, width) (array[((y) * (width) + (x)) >> 3] &= ~(MASK >> (((y) * (width) + (x)) % 8)))
// Extract the value of the bit (return 1 if the cell is ALIVE, 0 if the cell is DEAD)
#define GET_BIT(array, x, y, width)   (((array[((y) * (width) + (x)) >> 3] << (((y) * (width) + (x)) % 8)) & MASK) ? 1 : 0)
